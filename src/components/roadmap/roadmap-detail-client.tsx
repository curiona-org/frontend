...
export default function RoadmapDetailClient({
  ...
  useEffect(() => {
    const current = roadmap.rating?.rating ?? 0;
    setLocalRating(current);
    setInitialRating(current);
    // Jika rating awal > 0, anggap sudah rated
    setIsRated(!!roadmap.rating?.is_rated);
  }, [roadmap.rating]);
...
🔗 View this roadmap: <a href="https://curiona.netlify.app/roadmap/${roadmap.slug}">https://curiona.netlify.app/roadmap/${roadmap.slug}</a>`,
  ...
  // Handler klik bintang di UI utama
  const handleStarClickClient = (star: number) => {
    setLocalRating((prev) => (prev === star ? 0 : star));
    // tidak ada errorMessage di sini karena hanya rating
  };

  // Handler simpan rating dari UI utama
  const handleSaveRatingClient = async () => {
    // jika rating 0 atau sama dengan awal, tidak perlu simpan
    if (localRating === 0 || localRating === initialRating) return;
    ...
    try {
      ...
      setIsRated(true);
      // Update object roadmap.rating lokal agar UI segera mencerminkan update
      setRoadmap((prev) =>
        prev
          ? {
              ...prev,
      ...
      <div className="px-6 pt-24 lg:px-40 md:pt-32">
        <div className="bg-white-500 flex flex-col gap-6 border-2 border-blue-500 rounded-lg p-6">
          <div className="flex flex-wrap justify-between">
            <h1 className="w-full break-words lg:w-auto text-mobile-display-2 lg:text-heading-1-bold">
              {roadmap.title}
            </h1>
            {isLoggedIn && (
              <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-2">
                <Button
                  ...
                  aria-label="Rating"
                  className="flex items-center gap-2 p-2 border-2 border-white-600 md:hidden"
                >
                ...
                {isLoggedIn &&
                  ...
                  session.user.id === roadmap.creator.id && (
                    <Button
                      ...
                      className="text-gray-400 hover:text-blue-500 transition-all ease-out duration-300"
                    >
          ...
          <div className="text-mobile-body-1-regular lg:text-body-1-regular hidden md:flex fixed bottom-20 left-12 z-50 gap-4">
            ...
            <div className="flex items-center bg-white-500 border-2 border-blue-500 px-3 py-2 rounded-lg shadow-lg gap-2">
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <button
                  key={i}
                  onClick={() => handleStarClickClient(i)}
              ...
              {localRating > 0 && localRating !== initialRating && (
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setLocalRating(initialRating)}
                    disabled={ratingLoading}
                    className="px-2 py-1 rounded text-black-600"
                    aria-label="Cancel rating"
                  >
                    Cancel
                  </button>
                  <Button
                    onClick={handleSaveRatingClient}
                    disabled={ratingLoading}
                    className="ml-2"
                  >
                    {ratingLoading ? (
