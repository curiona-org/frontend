...
export default function GenerateRoadmap() {
  ...
  const handleGenerate = () => {
    ...
    setError("");
    router.push(
      `/personalization?topic=${encodeURIComponent(inputTopic.trim())}`
    );
  };
  ...
  return (
    <div className="w-full">
      <div className="relative w-full">
        <input
          type="text"
          maxLength={150} // Membatasi panjang input menjadi 150 karakter
          placeholder="Type here to generate your roadmap"
          className={`flex-grow w-full max-h-[83px] shadow-lg rounded-lg dashedBorder bg-white px-5 py-7 text-gray-700 text-mobile-heading-4-regular md:text-heading-4-regular focus:outline-blue-500 focus:bg-none focus:outline-none focus:ring focus:ring-blue-500 ${
            error ? "border-red-500" : ""
          }`}
          style={{ paddingRight: "80px" }} // Menambahkan padding untuk memberi ruang antara teks dan tombol
          value={inputTopic}
          onChange={(e) => {
        ...
        <button
          ...
          className="absolute right-5 top-1/2 -translate-y-1/2 rounded-lg bg-blue-500 p-3 text-white hover:bg-blue-900 focus:outline-none transition-transform duration-300 ease-out hover:scale-105 active:scale-95"
        >
          <svg
